...
본 프로젝트는 프로젝트 참여 인원이 적기 때문에 main, develop, feature 브랜치 위주로 이용합니다. 작업하기 전엔 다른 사람의 커밋을 최신으로 유지하기 위해 매번 `git pull && yarn` 을 실행하는 것을 권장합니다.
...
브랜치 이름은 `feature/new-submit-button`, `fix/#1/not-clicking-button`와 같은 형식으로 짓고, 설명 부분은 [Kebob Case](https://en.wiktionary.org/wiki/kebab_case)를 사용해서 영어로 간단히 짓습니다. 나중에 브랜치 단위로 코드 리뷰가 이뤄지므로 브랜치 단위는 코드 변경 사항이 최소 (+100)줄, 최대 (+500)줄이 되도록 설정하는 것을 권장합니다.
